...
// aliased to preact/compat in vite.config.ts.
const IGNORED_PACKAGES = new Set(['react', 'react-dom']);
// Imported by dist files that the four extensions never reach, so Vite never
// resolves them (verified via a --sourcemap build). If one of these becomes
// reachable, the Vite build fails on it — move it to dependencies then.
const UNREACHED_PACKAGES = new Set([
    '@enonic/page-editor',
    'ckeditor4-react',
    'd3',
    'd3-dag',
...
UNREACHED_PACKAGES.forEach((name) => IGNORED_PACKAGES.add(name));
